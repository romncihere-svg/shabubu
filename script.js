...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function triggerFireworksAndMemories() {
        ...
        setTimeout(() => {
            ...
            if (videoElement) {
                isVideoPlaying = true;
                // The video was preloaded muted (HTML attribute). Now unmute it and restart.
                // Using .muted = false is the reliable cross-host way to restore audio
                // after a user-gesture-gated play() call.
                videoElement.muted = false;
                videoElement.currentTime = 0;
                // Play returns a Promise — catch any remaining autoplay block gracefully.
                const playPromise = videoElement.play();
                if (playPromise !== undefined) {
                    playPromise.catch(err => {
                        console.warn('Video play error (may be autoplay policy):', err);
                        // Fallback: keep muted so it at least shows the video visually
    ...
    function executeHitAction(hit) {
        ...
        if (hit.name === "crystalHeart") {
            ...
            if (heartClickCount < HEART_CLICKS_NEEDED) {
                ...
                // We start the video IMMEDIATELY on click to lock in the user-gesture permission.
                // The video element already has the 'muted' HTML attribute so it plays silently
                // during the fireworks. We unmute it 4 seconds later when the heart reveals.
                const vid = document.getElementById('memoriesVideoPlayer');
                if (vid) {
                    vid.muted = true; // Ensure muted for the pre-play
                    let playPromise = vid.play();
                    if (playPromise !== undefined) {
                        playPromise.catch(err => console.log("Pre-play permission error:", err));
                    }
                    vid.pause(); // Pause right away; triggerFireworksAndMemories will restart from 0
                }
    ...
    function enterUniverse() {
        initSynth();
        // Start YouTube background music
        startBgMusic();
    ...
    function renderLoop() {
        ...
        if (!isVideoPlaying) {
            heartAnimTime += 0.016; // Approx 60fps delta
    ...
    // Memories Video 3D logic
    const memVideoPlayer = document.getElementById('memoriesVideoPlayer');
    if (memVideoPlayer) {
        memVideoPlayer.addEventListener('ended', () => {
            isVideoPlaying = false;
            // Fade out video inner heart
            gsap.to(innerHeartMat, { opacity: 0, duration: 1.5, ease: 'power2.inOut' });
            // Restore crystal color
            gsap.to(crystalMaterial.color, { r: 244/255, g: 114/255, b: 182/255, duration: 1.5 });
            // Restore scale
            gsap.to(heartGroup.scale, { x: 1, y: 1, z: 1, duration: 1.5, ease: 'power2.inOut' });
        });
    }
