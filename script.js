    ...
    function triggerFireworksAndMemories() {
        ...
        setTimeout(() => {
            ...
            if (videoElement) {
                ...
                videoElement.volume = 1;      // Turn the volume back up!
                videoElement.play();
    ...
    function executeHitAction(hit) {
        ...
        if (hit.name === "crystalHeart") {
            ...
            if (heartClickCount < HEART_CLICKS_NEEDED) {
                ...
                // but we set volume to 0 so you don't hear it during the fireworks.
                const vid = document.getElementById('memoriesVideoPlayer');
                if (vid) {
                    vid.volume = 0; // Silent (but not 'muted' property, which has different rules)
                    let playPromise = vid.play();
                    if (playPromise !== undefined) {
                        playPromise.catch(err => console.log("Autoplay permission error:", err));
                    }
                }
