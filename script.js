...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('Memories.mp4')
        ...
        .then(blob => {
            ...
            videoElement.load();
            if (typeof isVideoPlaying !== 'undefined' && isVideoPlaying) {
                videoElement.muted = false;
                videoElement.play().catch(e => { });
            } else {
                videoElement.play().catch(e => { }); // Play silently to buffer
            }
        })
    ...
    heartShapeGeo.computeBoundingBox();

    const bb = heartShapeGeo.boundingBox;
    const size = new THREE.Vector3();
    bb.getSize(size);

    const pos = heartShapeGeo.attributes.position;
    const uv = [];
    ...
    heartShapeGeo.setAttribute("uv", new THREE.Float32BufferAttribute(uv, 2));
    ...
    function revealVideoInHeart(bigScale) {
        ...
        // Force the material to recognize the video has started playing
        innerHeartMat.needsUpdate = true;
        ...
        gsap.to(innerHeartMat, { opacity: 1, duration: 2.0, ease: 'power2.inOut', onStart: () => { innerHeartMat.needsUpdate = true; } });
    ...
    function renderLoop() {
        ...
        // CRITICAL: Force VideoTexture update
